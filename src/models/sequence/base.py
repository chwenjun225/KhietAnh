...
class SequenceModule(nn.Module):
    ...
    def forward(self, x, state=None, **kwargs):
        ...
        Additionally, it returns a "state" which can be any additional 
        information. For example, RNN and SSM layers may return their 
        hidden state, while some types of transformer layers (e.g. Transformer-XL) 
    ...
    def state_to_tensor(self):
        ...
        This method should be implemented if one wants to use the hidden 
        state instead of the output sequence for final prediction.
    ...
    def step(self, x, state=None, **kwargs):
        ...
        this method should generally have signature (B, H1) -> (B, H2) 
        with an optional recurrent state.
        """
...
def TransposedModule(module):
    ...
    class TransposedModule(module):
        ...
        def forward(self, x: np.ndarray, state=None, **kwargs):
            if self.transposed: 
                x = x.transpose(-1, -2)
...
class SequenceIdentity(SequenceModule):
    ...
    def __init__(self, d_model, dropout=0.0, **kwargs):
        ...
        transposed: if True, inputs have axis ordering (B, H, L) instead of 
        (B, H, L)
        """
        super().__init__()
